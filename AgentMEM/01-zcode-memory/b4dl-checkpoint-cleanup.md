...
**同日备份与存档（2026-09-09 下午）**：精简后全部 1.7G（stage1 + B0/B3/B4a 顶层 adapter）已整包备份为 `backups/checkpoints_stage1_b0_b3_b4a_20260909.tar.gz`（1.4G，gzip -t 校验通过；backups/ 在 gitignore 内仅存服务器，本地 Windows 仓库无）；最优方案 B3 的可复现配方已文档化 `docs/learn docs/B4DL_B3最优训练方案存档_20260909.md`（commit 59fed9a 已 push）——含演进对照表、权威脚本配置（whole_scene+meta2 关键差异）、评测命令与全指标、产物路径。
