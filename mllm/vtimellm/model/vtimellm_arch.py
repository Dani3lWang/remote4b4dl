...
class VTimeLLMMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        # Optional B4DL extension. Default off so checkpoints produced before
        # this feature keep their original architecture and inference behaviour.
        ...
        self.config.use_frame_position_embedding = bool(use_frame_position_embedding)
        ...
        if self.config.use_frame_position_embedding:
            if not hasattr(self, 'frame_position_embedding'):
                ...
                # Zero init makes the first forward exactly match the B3
                # baseline, so turning the feature on is a single-variable change.
...
class VTimeLLMMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if getattr(self.config, 'use_frame_position_embedding', False):
            # Normalize both supported layouts to a per-sample list: the collator
            # yields a list for variable-length scenes, inference a [B, N, 768]
            # tensor (or [N, 768] for a single sample).
            ...
            if frame_indices is None:
                raise ValueError(
                    "frame_indices is required when use_frame_position_embedding=True"
                )
