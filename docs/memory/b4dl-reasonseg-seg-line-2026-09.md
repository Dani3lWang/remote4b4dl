...
三处必须一起引用的局限：① **选轮的量（dev `iou_mean`）与裁定的量（AUC/top-K）不是同一轴** —— ep5 的 AUC 更高（0.99395）、尺寸校准最好（11/12 = 0.92×），但 `iou_mean` 更低故未选中，**ep5 的 test 数不存在**；按 AUC 轴选轮只会让差距从 24× 收到 16×，裁定方向不变，但不在看过数据后改选轮规则。② **dev 参与选轮且是 in-distribution**（官方 train 场景内划分），test 是 unseen 官方 val，两者差一大截（top_k 0.2073 vs 0.1106），**对外只引用 test**。③ **与接地诊断不可直接相减**：诊断脚本当时仍带前缀掩码 bug（故意没改，待办 #20）。⚠ 该 bug 的偏差方向后来被实测纠正为**压低**而非抬高 AUC（详见第四节第 2 条），故这里"≥16× 是下界"的说法作废；按无偏口径重算，对照 0.8755 → ~4050 个负点、v3 解冻臂 0.99510 → ~159 个，改善约 **25×**。

### 解冻编码器臂（v3，09-25 23:55 `rc=0`）：有进展，但**这个问题还没被真正检验**

唯一变量是编码器解冻（1.49M 参数，独立 lr 1e-5），其余与 v2 逐字相同（同 manifest / 同 seed 故同洗牌顺序 / 6 epoch / head lr 1e-4 / 400 条评测 / Tversky+plain）。按 dev `iou_mean` **选定 ep5**（也是最后一轮）。
...
| iou_mean | 0.15856 | **0.19197** | +21% |
| 尺寸 pred/target | 29 / 12 | **17 / 12** | 过触发从 2.4× 收到 1.4× |

dev AUC 逐轮 **单调上升**：0.9839 → 0.9921 → 0.9927 → 0.9937 → 0.9946 → **0.9957**；train_loss 0.7958 → 0.5541 仍在降。**选定轮 = 最后一轮 ⇒ 是被 epoch 预算截断的，不是收敛的。**

按先验门槛换算（(1−AUC)×N，N≈32530）：v3 test 0.99510 → 每个正点上方约 **159** 个负点，需 ≤12 ⇒ **仍差约 13×**（v2 是 289 个 / 24×；对照无偏值 0.8755 → ~4050 个 / 338×）。

**两个实测出来的关键限定，别把 v3 读成"点特征已判死"：**

1. **编码器其实几乎没被微调。** 把选定轮的 `report_encoder.pt` 与预训练 `spatial-best/spatial_encoder.pt` 逐张量比：整体 ‖ΔW‖/‖W‖ = 18.28%，但拆开看 —— **卷积权重只动了 3.52%**、BN weight/bias 5.38%、而 **BN running_mean/var 动了 18.71%**（那是统计量，不是学出来的）。lr 1e-5 × 6 epoch 根本不足以让特征本身改变。⇒ **"点特征能否被改造成实例判别的"这个问题还没被检验**，v3 只证明了"动一点点就有可观收益"。
2. **v2 vs v3 不是干净的单变量消融。** 解冻同时把编码器切到 `train()` 模式，而 batch size = 1（单帧），BN 于是用**逐帧的 batch 统计量**归一化、running stats 漂移 18.7% —— 这一项**即使权重一个都不更新也会改变特征**。所以 +0.004 AUC / +72% top_k 里有多少来自"学习"、多少来自"BN 统计量适应"，目前分不开。

⇒ 正确的下一步是 v4：**提高编码器 lr**（1e-5 → 5e-5 或 1e-4，让卷积权重真的动）、**把 BN 钉在 eval 模式**（`train()` 之后逐个 `_BatchNorm.eval()`，冻结 running stats，消掉上面那个混淆）、**加长轮次**（AUC 还在单调升，6 轮明显不够）。并在每轮报告编码器权重的相对变化量，直接盯住"到底学没学"。

**一条自己的判读错误，记下来**：09-25 23:15 我在只看到 ep0–ep3 时报"AUC 没有逃出平台，解冻三轮约等于冻结五轮"。ep4/ep5 继续升到 0.9946/0.9957，test AUC 明确超过 v2。**在 loss 与指标都还在单调改善、且选定轮就是最后一轮时，不能报"平台"** —— 那只是预算用完。这与"判据挑错轴"是同一类错误的另一面：都是在证据不足时提前下裁定。
...
2. **诊断脚本把"框内点数"当"前缀长度"**（`point_count = point_valid.sum()` 后切 `[:point_count]`）：实测越界点占帧内 **6.18%**（中位 ~2035 点），但**尾部集中度只有 0.0585** ⇒ 越界点是散布的、不堆在文件尾，该近似不成立。后果：前缀内混入约 2035 个越界点（logit 被 mask 成 −1e4、概率恒 0），同时丢掉尾部约 2035 个框内点，**8/330 = 2.42% 的物体 GT 被截成空**；而 `diagnose_reasonseg_grounding.py` 里有 `if target_size == 0: continue`，这些物体被**静默跳过**（1000 条本应 1091 个物体、实报 1068，差的 23 个就是它们），也因此报告里看不到 NaN。
   - **⚠ 偏差方向曾被判错，09-26 已实测纠正**：原先（含本条记忆的早期版本）说"AUC 被约 2035 个必为 0 的简单负例**抬高**、真实值更低 ⇒ 只会加强 top-K 不可行的结论"。**实测相反 —— 修好口径后三个档的 AUC 全部升高**：对照 0.8576→**0.8755**、A2-ext20 0.8371→**0.8540**、B1 0.9048→**0.9252**；n 从 1068 降到 **1038**（`skipped_unreachable` = 53）。机制（从 per-object 数据核实）：旧口径下 AUC<0.6 的物体有 **116 个**，新口径只剩 **68 个**；`mean_prob_positive` 中位从 1.09e-3 翻倍到 **2.19e-3**。根因是前缀把**越界的 GT 点也当成了正例**，而它们的 logit 被 mask 成 −1e4、概率恒 0 —— 是"注定输给所有负例的正例"，把逐物体 AUC 拽向 0.5。当初只算了"白送的简单负例抬高 AUC"，漏了"不可能的正例压低 AUC"，而后者占主导。
   - **对结论的影响**：run 之间的比较一直成立（失真对所有 run 相同，这点没变）。绝对值修正后，对照的 top-K 缺口从"~4630 个负点"变成 **~4050 个**，仍是所需 ≤12 的 **338 倍** ⇒ "top-K 对完整模型不可行"这个裁定**不变**；但"这个修复会加强结论"那句话是错的 —— 它其实略微削弱了数字，只是没动摇量级。**教训：预测一个度量 bug 的偏差方向时，要把分子分母两侧的效果都算全，只算一侧就会把符号搞反。**
   - **处置**：探针（新代码，`aa8437e`）先用 in-range 掩码并把不可达物体计数上报；诊断脚本在 Phase 2.3 裁定完之后才改（`cdc29b9`，改用 `point_valid` 布尔掩码 + 长度断言 + `skipped_unreachable`），避免裁定前移动球门。旧报告全部保留，新报告写 `eval_results/_grounding_*_inrange/` 以便并排比。
