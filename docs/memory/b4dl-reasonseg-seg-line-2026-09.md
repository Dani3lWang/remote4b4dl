...
### v4（真微调编码器 + BN 钉 eval + 20 epoch，09-26 08:55:34 `rc=0`）：**单帧点特征撑不起实例接地，成立**

选定 ep15。消融干净性两项都过：`dW stats` 全程 **0.000%**（BN 钉 eval 生效，v3 的第二变量已消除）、`dW conv` 到选定轮 **19.97%**（ep19 22.41%），对比 v3 的 3.52% —— 这次特征真被大幅改造了。

test（oracle query，416 物体 + 23 不可达）三跑对比：
...
**dev AUC 在 ep5 见顶 0.99522，之后 14 轮在 0.9906–0.9947 震荡、ep19 收 0.99221，而同期 `dW conv` 从 11.6% 涨到 22.4% ⇒ 继续改造特征不再换来排序质量。** 命中预注册第三档（test AUC < 0.998 且 conv ≥ 15% 且已平）⇒ **裁定成立**。

两点必须一起记：
- **判据的局限**：`AUC ≥ 1−K/N` 是平均意义的最坏情况界，实测 top_k 0.339 远高于该界在 AUC 0.990 时的预测 ⇒ 排序误差是**偏斜**的（多数正例排得不错、少数极差），门槛偏保守；这解释了 v4 的 AUC 比 v3 低而 top_k 高 78%。**但裁定不依赖它** —— 完美 query + 真微调后仍只有 **24.8%** 的物体达到 IoU≥0.5（top-K 口径 33.9%），对照完整模型的自由生成 0.0727 / 无偏 TF 0.0886，即抬到约 2.8× 仍停在 1/4，不可用。这次 AUC 与 top-K 两轴**同向**。
- **选轮轴与判据轴第三次不一致**：按 dev `iou_mean` 选中 ep15（尺寸 23/12 = 1.9× 过触发），而 ep13 尺寸 **11/12 = 0.92×** 校准最好却没被选中。

### 语义质量的代价：v4 的编码器**不能直接插回完整模型**（线性探针，09-26 12:39）

同协议线性探针（冻结编码器且永久 eval、分类头从零训 2 轮、lr 1e-3、fp32、同 seed、train 25,324 / val 2,806）：

| 编码器 | miou | Δ vs 预训练 | point_acc | `dW conv` |
|---|---|---|---|---|
| `spatial-tv/spatial-best`（预训练） | **0.30785** | — | 0.90936 | — |
| v3 | 0.30560 | −0.00224（−0.7%） | 0.90818 | 3.52% |
| **v4** | **0.22073** | **−0.08712（−28.3%）** | 0.87906 | 19.97% |

三个都收敛（两轮间变化 ≤0.009），是真测量。**语义损失与 `dW conv` 严格同向 ⇒ 实例目标的微调与语义质量在打架。** 而完整模型里同一个编码器还要把场景压成 LM 的 scene token，所以 v4 那个把 instance recall 抬到 0.2476 的编码器**不能拿去替换 `spatial-tv`**。

出路按性价比：**① 多任务微调**（语义损失 + 实例损失一起，检验"两者能否共存于同一份特征"，约 8 h）＞ ② 双编码器（语义档供 compressor、实例档供掩码头，成本翻倍）＞ ③ 接受退化（与完整模型架构冲突，不可行）。**这条把"引入时序多帧"排到了多任务微调之后** —— 若语义与实例本质冲突，多帧只会让冲突更贵。
