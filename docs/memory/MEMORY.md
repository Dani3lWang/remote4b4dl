...
- [ReasonSeg seg 线全过程](b4dl-reasonseg-seg-line-2026-09.md) — 2026-09-18~25 autodl3：编码器换代 6.3×（0.0165→0.1037）后**七条杠杆全判死**；掩码塌缩机制（正例 3e-4）、A2 Tversky 治好尺寸却治不了实例接地（hit 24/330 逐位不动）、B1 切 LOC 先验落未决中间档；**oracle 探针：冻结特征下 test AUC 0.9911（差 24×），解冻后 0.9951（差 13×）且未收敛、卷积权重只动 3.5%，故"点特征是硬约束"只对冻结语义特征成立**；两个度量 bug（分母模型相关 / 前缀当 in-range 掩码）、以及**判据挑错轴这个复发三次的自身错误**；含 How to apply 与产物路径
