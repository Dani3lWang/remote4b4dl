...
**一句话现状**（09-26 01:20）：**GPU 已全空，两个作业都跑完了。** ① 探针 **v3（解冻编码器）** 23:55:34 `rc=0`，选定 ep5，test AUC **0.99510** / recall@0.5 **0.14423** / recall_top_k **0.18990** —— 相对 v2 冻结臂（0.99110 / 0.09615 / 0.11058）**每个轴都明显改善**（top_k +72%），但仍差约 **13×** 才到 top-K 可用门槛，且 **dev AUC 与 train loss 到最后一轮都还在单调改善 ⇒ 是被 6-epoch 预算截断的，没收敛**。② 无偏诊断复算完成，**偏差方向与我此前的说法相反**：新口径 AUC 全部升高（对照 0.8576→**0.8755**、A2 0.8371→**0.8540**、B1 0.9048→**0.9252**），但"top-K 对完整模型不可行"的裁定不变（仍有 203–396× 的缺口）。③ 实测编码器权重：卷积权重只动了 **3.52%**（整体 18.28% 主要是 BN 统计量漂移 18.71%）⇒ **"点特征能否变得实例判别"这个问题还没被真正检验**，且 v2/v3 之间还混进了 BN train 模式这个第二变量。下一步建议见 §6.4。
