...
**一句话现状**（21:50 更新）：Phase 2.3 主链与 oracle 探针 v2 都已跑完并裁定 —— ① B1 臂落在预注册判据的**未决中间档**，held-out mean IoU 低于对照；② v2（冻结编码器）test AUC **0.99110**，按先验判据（需 ≳0.99963）**仍差约 24×**，且"改 `voxel_size`"这条已被实测推翻（§3.4：体素并列 T 中位为 0，AUC 上限 0.99999996）。**当前在跑 v3 = 解冻编码器臂**（tmux `p24`，ETA ~23:55），因为 v2 量的是"现有语义特征 + 完美 query"的上限、不是点特征本身的上限；v3 之后由 tmux `rediag` 自动用修好口径的诊断复算三个档（ETA ~00:20）。详见 §1.1。
