...
### 6.1 适配性验证（2026-08-29，`validate_encoder_fit.py`）

同一批 nuScenes 数据（1,600 帧，与训练同分布）上对比新编码器与旧 ONCE 权重对 CLIP 图像特征（ViT-L/14，fp32）的贴合度：

| 编码器 | 对齐 MSE ↓ | 相似度结构相关 ↑ | 特征范数 | 加载 |
|---|---|---|---|---|
| **新 nuScenes（step 19500）** | **0.0830** | **0.620** | 16.47 | 119/119 全干净 |
| 旧 ONCE 权重 | 0.1693（2.0×） | 0.599 | 16.22 | 6 个多余键（旧格式遗留） |

结论：**新编码器在 nuScenes 上的对齐损失减半、语义结构略优、加载零缺陷**——domain gap 的量化实锤（旧权重 MSE 0.169 即从未对齐过本数据），适合按论文方法进入重提特征 → projector 重训 → 混合重训流程。
