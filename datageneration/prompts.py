...
class Prompts:
    ...
    def generate_temporal_understanding_dataset_prompt(self, front_description, back_description, gt_caption, start_index, end_index):
        # 注：此处每序列请求 10 条。论文 §3.3 声称 Temporal Understanding 每序列
        # 5 条，但按发布版 23,956 条 / 4,200 序列 ≈ 5.7 条落地率反推，官方管线
        # 实际请求量为 10（GPT 解析损失 ~57% 落地），维持 10 才能对齐发布数据。
    ...
    def generate_time_grounding_dataset_prompt(self, front_description, back_description, gt_caption, start_index, end_index):
        ...
        TIME_GROUNDING_DATASET_PROMPT = f"This is a description of the front parts of the ego vehicle from frame {start_index} to frame {end_index}: {front_description}. This is a description of the back parts of the ego vehicle from frame {start_index} to frame {end_index}: {back_description}. Ensure the model considers the back view correctly: objects on the left correspond to the ego vehicle’s right, and objects on the right correspond to its left, based on the LiDAR perspective. Generate 5 time-grounding question-answer pairs about the scene. Each question asks WHEN a specific event happens (e.g., when an object appears, starts moving, stops, becomes visible, or when the ego vehicle performs an action), and each answer must be a single temporal span in exactly this format: 'from frame 000 to frame 000.' with three-digit zero-padded frame numbers and a trailing period. Ground every span in the frame numbers appearing in the descriptions; never invent events that are not described. The format of the question-answer pair should be as follows: Q: What is the question? A: from frame 000 to frame 000. Just create a question-answer pair without any other embellishments or explanations. Use only Q and A; Do not use Q1 and A1, Q2 and A2, etc. for the question and answer pairs. Below are some examples: Q: When is the overpass visible to the ego vehicle? A: from frame 006 to frame 014. Q: When did a pedestrian begin crossing the street towards the ego vehicle's direction? A: from frame 000 to frame 008. Q: At which frames does a path to the left become more visible in the front view? A: from frame 018 to frame 020."

        if gt_caption:
            TIME_GROUNDING_DATASET_PROMPT = TIME_GROUNDING_DATASET_PROMPT + f" The following is part of the ground truth for this sequence: {gt_caption}. Use this information if necessary."

        return TIME_GROUNDING_DATASET_PROMPT
